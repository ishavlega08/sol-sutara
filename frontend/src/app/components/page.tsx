        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <select value={riskFilter} onChange={(e) => { setRiskFilter(e.target.value); setPage(1); }}
              className="appearance-none rounded-md border border-gray-200 bg-white py-1.5 pl-3 pr-7 text-xs text-gray-600 outline-none hover:bg-gray-50">
          ...
          <div className="ml-auto flex items-center rounded-md border border-gray-200 bg-white p-0.5">
            {(["table", "graph"] as const).map((m) => (
              <button key={m} onClick={() => setViewMode(m)}
                className={`flex items-center gap-1 rounded px-2.5 py-1 text-xs font-medium transition ${viewMode === m ? "bg-gray-100 text-gray-900" : "text-gray-400 hover:text-gray-600"}`}>
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-gray-400">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
                className="flex items-center gap-0.5 rounded border border-gray-200 px-2 py-1 text-xs text-gray-500 hover:bg-gray-50 disabled:opacity-40">
