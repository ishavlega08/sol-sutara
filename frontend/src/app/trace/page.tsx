...
export default function TracePage() {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden lg:flex-row">
        ...
        <div className="flex-shrink-0 border-b border-gray-100 px-4 py-4 sm:px-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-gray-400">Walk the lineage of any component, upstream or downstream.</p>
            </div>
            <div className="flex flex-shrink-0 flex-wrap items-center gap-2">
              <div className="flex items-center rounded-md border border-gray-200 bg-white p-0.5">
                {DIR_BTNS.map((d) => (
                  <button key={d} onClick={() => setDirection(d)}
                    className={`rounded px-2.5 py-1 text-xs font-medium transition ${direction === d ? "bg-gray-100 text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
                  >
      ...
      {/* Right panel */}
      <div className="flex w-full flex-shrink-0 flex-col divide-y divide-gray-100 border-t border-gray-200 bg-white overflow-y-auto lg:w-80 lg:border-l lg:border-t-0">

        <div className="px-5 py-4">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900">Trace result</h2>
          ...
          <div className="flex flex-col gap-2.5 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-emerald-600">{"< 200ms"} <span className="text-[10px] font-normal text-gray-400">★</span></span>
            </div>
            <p className="text-[10px] text-gray-400">★ devnet estimate</p>
          </div>
        </div>

        <div className="px-5 py-4">
          <h2 className="mb-3 text-sm font-semibold text-gray-900">Selected node</h2>
          {selected && selectedId ? (
            <div className="flex flex-col gap-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-400">{selected.tier} · {selected.org}</p>
              </div>
              <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{selected.parents}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-500">Children</span>
                  <span className="font-semibold text-gray-900">{selected.children}</span>
                </div>
              </div>
              <Link href={`/components/${selectedId}`}
                className="mt-1 w-full rounded-md border border-gray-200 py-2 text-center text-sm text-gray-700 transition hover:border-gray-300 hover:bg-gray-50">
                Open detail →
              </Link>
            </div>
          ) : (
            <p className="text-xs text-gray-400">Click a node to inspect it.</p>
          )}
        </div>
